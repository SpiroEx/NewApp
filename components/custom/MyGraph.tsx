...
const MyGraph: React.FC<MyGraphProps> = ({ myUser }) => {
  //! LOGS for graph
  const [logs, loadingLogs] = useFHWatchQuery(FH.MyUserLog(myUser?.id ?? ""), [myUser?.id], orderBy("id", "asc"))
  ...
  }, [filtered, pefLineData, fev1LineData, fvcLineData, fev1FvcLineData])

  const filterOnClick = (index: number) => {
    if (filtered === index) {
      setFiltered(null)
      return
    }
  ...
  const colors = ["bg-[#8884d8]", "bg-[#82ca9d]", "bg-[#ffc658]", "bg-[#ff7300]"]
  ...
  return (
    <div className="wf csc-5">
      ...
      >
        <Chart_Line lines={graphs} />
      </GraphData>

      {/*//! IMPROVING / NOT */}
      <div className="css-4 w-60">
        <div className="grid grid-cols-1 gap-3 px-2 w-80">
          <button className="r-20 b-2 border-color-white rounded-xl" onClick={() => setFiltered(null)}>
            Clear Filters
          </button>
          {[
            ...
            <span key={i} onClick={() => filterOnClick(i)} className="flex gap-2 items-center">
              <div
                className={`w-3 h-3 ${filtered === i ? colors[i] : "bg-slate-300"} ${filtered === null ? "bg-slate-50" : "bg-slate-300"} rounded-full cursor-pointer `}
              ></div>
              {filtered === null || filtered === i ? <p className="t43 o-75 hover:o-100">{text}</p> : <p className="t43 o-50 hover:o-50">{text}</p>}
            </span>
          ))}
        </div>
      </div>
