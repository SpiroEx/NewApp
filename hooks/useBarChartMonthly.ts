...
>(
  ...
  const [lastShownMonth, setLastShownMonth] = useState(
    DateHelper.getLastMonthAbbrev(currentYear)
  );
  ...
  useEffect(() => {
    ...
    if (currentYear) {
      setLastShownMonth(DateHelper.getLastMonthAbbrev(currentYear));
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < monthsShownLength; i++) {
      ...
      month = DateHelper.getPrevMonthAbbrev(month);
      if (month === "Dec") {
        year--;
  ...
  function prev() {
    const monthIndex = DateHelper.monthAbbrev.indexOf(lastShownMonth);
    if (monthIndex === 0) {
      setLastShownMonth("Dec");
      setLastShownYear(lastShownYear - 1);
    } else {
      setLastShownMonth(DateHelper.monthAbbrev[monthIndex - 1]);
    }
  }
